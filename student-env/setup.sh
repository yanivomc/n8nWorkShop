...
menu() {
  while true; do
    ...
    echo "  ══════════════════════════════"
    echo "  ── Setup  (run in this order first time) ─────"
    echo "  1) Configure cluster        (IP + kubeconfig)"
    echo "  2) Configure API keys       (Gemini, Telegram, TOTP, ngrok)"
    echo "  3) Start stack              (n8n + MCP)  ← needs 1-2"
    echo "  4) Start ngrok tunnel       (Telegram HTTPS webhook only)"
    echo "  5) Install Prometheus + Grafana  ← uses EC2 IP, not ngrok"
    echo "  6) Deploy Dashboard + Target App to Kubernetes"
    echo "  ── Re-run when things change ─────────────────"
    echo "  2→5) EC2 IP changed?        re-run 2, then re-run 5"
    echo "  4)   ngrok domain changed?  re-run 4 only (Prometheus unaffected)"
    echo "  ── Operations ────────────────────────────────"
    echo "  7) Stop stack"
    echo "  8) Validate full setup      (17 checks — all must pass)"
